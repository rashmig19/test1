...
def get_case(interaction_id: str) -> Dict[str, Any]:
    ...
    """
    url = (getattr(settings, "GET_CASE_URL", "") or "").strip()
    if not url:
        raise RuntimeError("GET_CASE_URL is not configured")

    params = {
        "interactionId": interaction_id   # change key name if API expects something else
    }
    ...
    logger.debug("Calling GET_CASE_URL=%s params=%s", url, params)
    resp = requests.get(
        url,
        params=params,
        headers=headers,
...
def sync_case_from_interaction(interaction_id: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    Convenience function:
      ...
      2) Strip all 'px*' keys.
      3) Call create_case with the cleaned payload.
    ...
    return raw_case, cleaned_case

# ========================================

# --- NEW: Get Case -> strip px* keys -> Create Case ---
    try:
        raw_case, filtered_case = sync_case_from_interaction(interaction_id)
        session_state[thread_id]["raw_case_payload"] = raw_case
        session_state[thread_id]["filtered_case_payload"] = filtered_case
    except Exception as e:
        # Do not break main PCP flow if case APIs fail
        logger.error(
            "Case sync failed for interaction_id=%s (thread_id=%s): %s",
            interaction_id,
            thread_id,
            e,
            exc_info=True,
